...
// Updated on Wednesday, August 3, 2011.
// 3 Major concerns addressed:

// First Concern: When the longest word in "Word-List.txt" doesn't start with an 'A'.
// int IndexCount = BreadthQueueUseToIndex(OrderMatters, DawgRootNode(TemporaryTrie));
// DawgRootNode(ThisTrie) Simply returns a pointer to the root node in "ThisTrie".

// Second Concern: Case Insensitivity => MakeMeAllCapital(ThisLine) is now used when reading from "Word-List.txt".

// Third Concern: If some characters in the character-set are NOT used to start any words:
// When using the compiled DAWG, just use an array to calculate the node to start from. (see lines below) - It's for use and speed.
// When not using (B,C,F,K,P,X,Y) to start words:
// int FirstNodeIndex[26] = { 1, 0, 0, 2, 3, 0, 4, 5, 6, 7, 0, 8, 9, 10, 11, 0, 12, 13, 14, 15, 16, 17, 18, 0, 0, 19 };
